...
1. ในสถาปัตยกรรมของ Kestrel ตัวแปร `builder` ทำหน้าที่อะไร และตัวแปร `app` ทำหน้าที่อะไร<br> 
		ตอบ ตัวแปร `builder` ใช้สำหรับเตรียมและตั้งค่าต่าง ๆ ของ Web Server เช่น การกำหนด Service และการตั้งค่าของโปรแกรม ก่อนที่จะเริ่มทำงานจริง ส่วนตัวแปร `app` ใช้สำหรับกำหนดการทำงานของ Web Server หลังจากสร้างเสร็จแล้ว เช่น การสร้าง Route หรือ Endpoint ต่าง ๆ และสั่งให้ Server เริ่มทำงาน สรุปง่าย ๆ คือ `builder` ใช้สำหรับ **เตรียมและตั้งค่า** ส่วน `app` ใช้สำหรับ กำหนดการทำงานและรับ Request จากผู้ใช้งาน
2. เปรียบเทียบความสะดวกระหว่างการสร้าง Web Server บน .NET Minimal API กับการรันผ่าน LAMP Stack (Apache + PHP) ว่ามีข้อดีข้อเสียต่างกันอย่างไรในมุมมองของงาน IoT Gateway<br>
		ตอบ ถ้าเป็นงาน IoT Gateway ผมคิดว่า .NET Minimal API สะดวกกว่า เพราะสามารถสร้าง API ได้ด้วยโค้ดไม่กี่บรรทัด และไม่จำเป็นต้องติดตั้ง Apache หรือ PHP เพิ่ม ทำให้เหมาะกับงานที่ต้องการให้ ESP32 ส่งข้อมูลเข้ามาที่ Server ข้อดีของ .NET Minimal API คือโครงสร้างไม่ซับซ้อน เริ่มต้นใช้งานได้เร็ว และสามารถเขียน API สำหรับรับข้อมูลจาก Sensor ได้โดยตรง นอกจากนี้ยังเหมาะกับการทำงานที่ต้องการความรวดเร็วในการพัฒนา ส่วน LAMP Stack มีข้อดีตรงที่เป็นระบบที่มีคนใช้งานมานาน มีตัวอย่างและเครื่องมือให้ศึกษาเยอะ โดยเฉพาะถ้าต้องทำเว็บไซต์ที่ใช้ PHP แต่สำหรับงาน IoT Gateway อาจต้องตั้งค่าหลายส่วนมากกว่า และมีส่วนประกอบที่ไม่จำเป็นสำหรับการทำ API ขนาดเล็ก ดังนั้น ถ้าเป็นงาน IoT Gateway ที่เน้นรับและส่งข้อมูลจาก ESP32 ผมมองว่า .NET Minimal API เหมาะกว่า เพราะติดตั้งและพัฒนาได้ง่ายกว่า
3. นักศึกษาคิดว่าการเพิ่ม `/api/` เข้าไปใน route นั้นมีประโยชน์อย่างไรบ้าง ถ้าไม่ใส่จะเกิดปัญหาอะไรบ้าง<br>
		ตอบ การเพิ่ม `/api/` เข้าไปใน Route ช่วยให้เรารู้ว่า URL นั้นใช้สำหรับ API โดยเฉพาะ ทำให้แยกส่วนของ API ออกจากหน้าเว็บไซต์หรือส่วนอื่น ๆ ของระบบได้ง่ายขึ้น เช่น `/api/student` และ `/api/status` ทำให้เวลาโปรเจกต์มี Endpoint เพิ่มขึ้นหลายตัวก็ยังจัดการได้ง่ายและดูเป็นระบบ ถ้าไม่ใส่ `/api/` ก็ไม่ได้ทำให้ระบบเกิดปัญหาทันที เพราะ API ยังสามารถทำงานได้ตามปกติ แต่ถ้าโปรเจกต์มีหลายหน้าและมีหลาย Endpoint อาจทำให้ URL สับสนหรือแยกประเภทของการใช้งานได้ยาก ดังนั้นการใช้ `/api/` จึงช่วยให้โครงสร้างของระบบเป็นระเบียบมากขึ้น
